...
def AUC(y_pred, y_true):
  # 使用softmax函数将y_pred转换为概率
  probs = torch.softmax(y_pred, dim=1)
  # 获取样本属于正类的概率预测值
  positive_probs = probs[:, 1]
  # 计算AUC
  auc = roc_auc_score(y_true.detach().numpy(), positive_probs.detach().numpy())
  return auc
...
def AUPRC(y_pred, y_true):
    ...
    positive_probs = probs[:, 1]
    # 计算AUPRC
    auprc = average_precision_score(y_true.detach().numpy(), positive_probs.detach().numpy())
    return auprc
...
def build_args():
    ...
    parser.add_argument("--dataset", type=str, default="PANCAN")
    ...
    parser.add_argument("--max_epoch_f", type=int, default=50)
    ...
    parser.add_argument("--num_features", type=int, default=1604)

    # for edgeGAE

    parser.add_argument("--mask", nargs="?", default="Edge",
                        help="Masking stractegy, `Path`, `Edge` or `None` (default: Path)")
    parser.add_argument('--seed_e', type=int, default=2024, help='Random seed for model and dataset. (default: 2022)')
    ...
    parser.add_argument('--decoder_dropout', type=float, default=0.1,
                        help='Dropout probability of decoder. (default: 0.1)')
    parser.add_argument('--alpha', type=float, default=0.5, help='loss weight for LOSS. (default: 0.5)')

    parser.add_argument('--lr_e', type=float, default=0.01, help='Learning rate for training. (default: 0.01)')
    parser.add_argument('--weight_decay_e', type=float, default=5e-5,
    ...
    parser.add_argument('--q', type=float, default=0.1, help='Mask ratio or sample ratio for MaskNode')
    ...
    parser.add_argument('--eval_period', type=int, default=100, help='(default: 30)')
