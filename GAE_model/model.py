...
class GNN(nn.Module):
    ...
    @torch.no_grad()
    def get_embedding(self, x, edge_index, mode="last"):
...
class MaskGAE(nn.Module):
    ...
    def train_step(self, data, optimizer, alpha=0.5,
        ...
        for perm in DataLoader(
            ...
            z1 = self.encoder(x, remaining_edges)
            z2 = self.encoder(masked_x, edge_index)
            ...
            pos_out = self.edge_decoder(
                z1, batch_masked_edges, sigmoid=False
            )
            neg_out = self.edge_decoder(z1, batch_neg_edges, sigmoid=False)
            loss1 = self.loss_fn(pos_out, neg_out)
            # **********************************************************************

            # ******************* loss for node prediction ***********************
            loss2 = sce_loss(self.node_decoder(z2).squeeze(), x)
            # **********************************************************************
